...
private:
    ...
    void requestBaud(const HW_Port portIndex, const HW_Baud baudRate) WARN_IF_UNUSED;
    // Send a request to the GPS to enable a message type on the port.
    void requestGSOF(const uint8_t messageType, const HW_Port portIndex, const Output_Rate rateHz) WARN_IF_UNUSED;
    
    void disableOutput(const HW_Port portIndex) WARN_IF_UNUSED;
